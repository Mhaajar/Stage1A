...
function Home() {
  ...
  // Fetch Data Fuction
  const fetchData = () => {
    axios("http://localhost:5000/api/demandes/")
      .then((res) => {
  ...
  const getDemandeByID = (id) => {
    axios(`http://localhost:5000/api/demandes/getDemande/${id}`)
      .then((res) => {
        setDateCreation(res.data.DateCreation);
        setTypeAnalyse(res.data.typeAnalyse);
        setNomPatient(res.data.Patient.NomFamille);
        setPrenomPatient(res.data.Patient.Prenom);
        setBirth(res.data.Patient.birth);
        setAdress(res.data.Patient.adress);
        setSex(res.data.Patient.sexe);
        setPhone(res.data.Patient.phone);
        setAge(res.data.Patient.age);
        setCin(res.data.Patient.cin);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  //Delete function
  const delete_demande = (id) => {
    if (window.confirm("Are you sure you want to delete this Demande ?")) {
      axios
        .delete(`http://localhost:5000/api/demandes/delete/${id}`)
        .then((res) => fetchData())
        .catch((err) => console.log(err));
    }
  };

  const update_demande = () => {
    let data = {
      DateCreation: DateCreation,
      typeAnalyse: typeAnalyse,
    };
    axios
      .patch(
        `http://localhost:5000/api/demandes/updateDemande/${demandeID}`,
        data
      )
      .then((res) => fetchData())
      .catch((err) => console.log(err));
